  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {skillsData.map((group) => (
        <div
          key={group.name}
          className="rounded-2xl border border-orange-300/15 bg-gradient-to-br from-orange-500/5 via-neutral-950/80 to-sky-500/8 p-5 shadow-[0_15px_40px_rgba(0,0,0,0.35)] backdrop-blur"
        >
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
            {group.skills.map((skill) => (
              <div
                key={skill.name}
                className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-100 transition hover:border-orange-300/50 hover:bg-orange-500/10"
              >
